...
- ✅ `bc2df19` Tramo 2 iter 1 · `POST /projects/{id}/close` (guard: sin intervenciones abiertas salvo force · snapshot en `project_reports` · audit `project.close`) + `GET /projects/{id}/report(.html|.csv)` informe de avance/cierre en español con horas locales del sitio, desvío de llegada, duración, SLA, equipos con serial, canceladas y pendientes · botón "Cerrar proyecto" + opción PDF del servidor en RolloutDetailPage · verificado en PROD (58 visitas · 51 instaladas · 63 equipos · 86.3 % puntualidad · 96.1 % SLA · 2h36m medio en sitio)
- ✅ `4bb4446` Producto modular · sidebar SRS en 3 módulos (Coordinación: cockpit/mapa/intervenciones/proyectos · Operaciones: sitios/técnicos · Administración: finanzas/contratos/inteligencia/admin) con contador "pendiente de cerrar" por módulo (`GET /work-orders/module-counts`: coord = intake/triage/resolved + sin programar · ops = en campo · admin = cerradas sin facturar)
- ⏳ Tramo 2 pendiente: alcance real del rollout con Andros (dashboard marca 50.5 % porque el objetivo SOW son 101) · el cierre lo pulsa el coordinador desde Rollouts cuando Andros diga · owner ya no quiere "demo": el espacio Cliente queda como está
- ✅ `dce8925`+`83b4d35` Administración cierra su parte (owner: "dale a la de Administración, la valida Adriana") · `scripts.close_admin_arcos_panama` (DRY-RUN/`ADMIN_EXECUTE=1`, actúa como owner vía las mismas funciones de las rutas): Alarmas Solutions → `vendor_labor` · `cost_snapshot.labor=300` en 50 visitas Panamá (tarifa Alarmas/Agustín · pendiente factura real) · pre-factura `INV-202510-F7D6-001` Claro CES PA-1000066 50 × $500 = $25,000 draft · pre-factura `INV-202512-F7D6-001` PA-1000055 FM-19566 $500 draft · suscripción MRC 50 sitios × $28.33 = $1,416.50/mes (next_run 2026-10-01) · P&L nominal 40 % ($10,000) · `generate` acepta `project_id`/`work_order_ids`
- ✅ `789c2cd`+`bb04a77`+frontend Tramo 3 · **Modo 5 survey** (owner: "dale al Tramo 3, el survey"): `report_templates` versionadas (modelo + rutas + seed `claro_us_wireless_survey` v1.0 · 11 secciones · 22 campos obligatorios · absorbe el PDF Charleston + los 3 huecos de Bepensa: fotos de ubicación de APs nuevos, evaluación Cat6, proyección de puertos) · `project.report_template_id` (survey sin plantilla → 400) · `POST /projects/{id}/generate-visits` (1 WO por sitio) · captura con `template_responses` · **playbook enforced**: `on_site→resolved` bloquea si faltan obligatorios (emergency override) · entregable por sitio `GET /work-orders/{id}/deliverable(.html)` en español con fotos incrustadas, co-brand JV automático, estrellas, visible al cliente · PWA: bloque "Playbook" con progreso, campos por tipo (texto, número, sí/no, opción, estrellas, fotos por campo), bloqueo del botón TERMINÉ hasta completar · Rollouts: chip de plantilla, "Generar visitas" (sitios de la org sin visita), botón "Entregable" en tarjetas resueltas/cerradas · lista incluye type=survey · smoke test por API en la org DEMO: 12/12 pasos OK (bloqueo con 20 campos faltantes → completo → resolved → entregable 200 para SRS y cliente)
- ⏳ Survey real: cuando se firme la oferta VE/PA/Caribe crear el proyecto type=survey con el SA real + plantilla (ajustar campos al molde que pida Claro) + cargar sitios (bulk upload) + generar visitas · LCON approval, correction_items y narrativa Copilot quedan para después (decisiones Modo 5 #3, #6, #10)
- ⏳ Adriana valida en Finanzas: revisar líneas y recargos SOW E.6 (after-hours/fin de semana no aplicados) → Enviar cuando Claro la reciba · registrar las facturas reales de Alarmas (Pagos a proveedores) para que el P&L pase de estimado a real · revisar la suscripción MRC (60 meses por sitio desde instalación · hoy agregada en una sola)
