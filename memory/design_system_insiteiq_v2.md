...
> **Status:** v1.8 · 2026-04-29 · **DEPLOYED EN PROD** vía sprint de reanudación (Alpha→Eta). Tres vistas v2 vivas en `insiteiq.systemrapid.io/srs?v2=1`, `/srs/espacio-ops?v2=1`, `/srs/intervenciones?v2=1`. Icon set Solar Linear. Arquitectura SRS dark con tres vistas estructuralmente diferentes (Cockpit cards+widgets, Espacio OPS mapa+detail, Kanban drag&drop+modal). Referencia validada 1:1 con SKYPRO360 OpsManager en PROD. Toggle `?v2=1` activo durante semana de validación con Andros + JuanCho antes de cambiar default a v2.
>
...
- **2026-04-29 · v1.8 · JuanCho + Claude** — **DEPLOYED EN PROD VPS 1.** Sprint de reanudación completado en sesiones consecutivas: 7 fases (Alpha foundation, Beta shell, Gamma cockpit, Delta espacio OPS, Epsilon kanban, Zeta polish, Eta deploy) cerradas con 30 tasks completadas. Build local validado (1675 módulos · 3.35s · gzip 673KB). Smoke test PROD OK: las 3 vistas v2 renderizan con data real del backend. Componentes deployed en `frontend/src/components/{cockpit-v2,warroom-v2,kanban-v2,shell-v2,v2-shared}/` y páginas en `frontend/src/spaces/srs/v2/`. Iconify Solar Linear vía CDN. Leaflet vía CDN para War Room mapa. Drag&drop nativo HTML5 sin librerías. RefreshContext compartido para polling indicator pulsante en header. V2ErrorBoundary envolviendo cada vista. Skeleton states + EmptyState + filter dropdowns funcionales con MultiSelect. Toggle de activación: `?v2=1` query param hasta switch global por env var `VITE_V2_SHELL=1` cuando Andros + JuanCho firmen una semana de validación con datos reales. Iconography catálogo §3.6 ahora vivo en `lib/icons.jsx` (helper `<Icon icon={ICONS.search} />` + catálogo cerrado de glyphs). Timezone-aware §3.6a vivo en `lib/tz.js` (TECH_REGISTRY + getTechTimeInfo) usado en bottom strip + popup mapa + panel detail. Drag handle 6-dots §3.6b vivo en WoKanbanCard. Backend intacto — cero modificaciones a endpoints, schemas, rutas FastAPI durante el sprint completo.
